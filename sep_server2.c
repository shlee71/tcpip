...
int main( int argc, char **argv)
{
    ...
    printf("accept() : message from client ip[%s] is received \n\n", inet_ntoa(clnt_addr.sin_addr));

    rstrm = fdopen(clnt_sock, "r");
    // create file descriptor using dup copied fds
    wstrm = fdopen(dup(clnt_sock), "w");

    // Server message send 
    fputs("From Server : Hello ?\n", wstrm);
    fputs("I like network programing.\n", wstrm);
    fputs("Do you like ?\n", wstrm);
    
    // 1st close output socket & Acknowledge client the end of output
    fflush(wstrm);
    shutdown(fileno(wstrm), SHUT_WR);
    fclose(wstrm);

    // expect input stream from client
    fgets(buf, sizeof(buf), rstrm);
    fputs(buf, stdout);

    // 2nd close input socket
    fclose(rstrm);
