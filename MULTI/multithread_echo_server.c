...
int thread_index = 0;
int thread_id[MAX_THREAD_LIMIT];
/*
typedef struct {
    int clnt_sock;
    int t_index;
...
thread_info mythread[MAX_THREAD_LIMIT];; 
*/
...
int main( int argc, char **argv)
{
    ...
    while( 1 )
    {
        ...
        if ( child_process_cnt < MAX_THREAD_LIMIT)
        {
            ...
            //mythread[thread_index].alive = false;  
            //mythread[thread_index].clnt_sock = clnt_sock;            
            //mythread[thread_index].t_index = thread_index;
            pthread_mutex_unlock(&mutx);

            state = pthread_create(&thread_id[thread_index], NULL, echo_client_thread, (void *)clnt_sock);
            //state = pthread_create(&mythread[thread_index].t_id, NULL, echo_client_thread, (void *)&mythread[thread_index]);
            if (state != 0)
            {
