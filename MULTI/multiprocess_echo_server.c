...
int main( int argc, char **argv)
{
    ...
    while( 1 )
    {
        ...
        if ( child_process_cnt < MAX_CHILD_PROC)
        {
            pid = fork();
        }
        else
        {
            printf("Maximum Child Process LIMIT count[%d] reached !!!\n\n", child_process_cnt);
            continue;
        }
...
void z_handler(int sig)
{
    ...
    while ( pid = waitpid(-1, &rtn, WNOHANG) > 0)
    {
        ...
        child_process_cnt--;
        printf("Child Process released, child count ==> [%d] !!!\n\n", child_process_cnt);
    }
}